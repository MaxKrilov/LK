...
let SpeedComponent = SpeedComponent_1 = class SpeedComponent extends Vue {
    constructor() {
        super(...arguments);
        // Data
        this.chartHeight = 0;
        this.strokeWidth = STROKE_WIDTH;
        this.endPieAngel = END_PIE_ANGLE;
        this.chartLayer = null;
        this.pieG = null;
    }
    // Computed
    get computedId() {
        return `er-speed-component--${this._uid}`;
    }
    // Methods
    init() {
    ...
    draw() {
        ...
        function arcTween(d, i, el, start) {
        ...
        label
            ...
            .attr('y1', 9)
            .attr('x2', 0)
            .attr('y2', (d) => d.data % 10 === 0 ? 0 : 6)
            .attr('transform', (d) => `translate(${(arcRadius + 14) * Math.sin(d.startAngle)}, ${(arcRadius + 14) * -Math.cos(d.startAngle)}), rotate(${d.startAngle * 180 / Math.PI})`)
            .attr('stroke', 'rgba(0, 0, 0, 0.17)')
            .attr('stroke-width', 2)
            .classed('scale-tick', (d) => d.data % 10 === 0 && d.data > currentSpeed && d.data !== lastAvailableSpeedVal);
        label
        ...
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        function setCursorNewPosition(pos) {
            /* mainCursorSelection
        ...
        const setScaleNumbersColors = () => {
            ...
            d3.select(`#${this.computedId}`)
                ...
                .filter((d) => d.data > currentSpeed && d.data !== lastAvailableSpeedVal)
        ...
        const getSpeedFromAngle = (coords) => {
        ...
        const setSpeed = (speed) => {
            if (speed < currentSpeed)
                speed = currentSpeed;
            else if (speed > getLastElement(listSpeed))
                speed = getLastElement(listSpeed);
        ...
        d3.select(`#${this.computedId}`).select('#arc-0')
            ...
            if (isOnTurbo)
                return;
            const coords = d3
                .mouse(document.querySelector(`#${this.computedId}`).querySelector('.main-field'));
...
SpeedComponent = SpeedComponent_1 = __decorate([
    Component({
        model: {
            event: 'change'
        },
