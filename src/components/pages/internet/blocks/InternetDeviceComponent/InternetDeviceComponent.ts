...
@Component
export default class InternetDeviceComponent extends DeviceComponent {
    ...
    this.customerProducts({ api: this.$api, parentIds: [this.productId] })
      .then((response) => {
        ...
        this.devices = devices.map((device: any) => {
          ...
          _device['Тип оборудования'] = device.chars?.['Тип оборудования'] || 'Роутер'
          _device['Модель'] = device.chars['Модель']
