...
er-dialog-advanced(:active="active", icon="doc", @close="$emit('close')")
  ...
  a(
    ...
    data-ga-label="userslistlaw"
  ) п. 2 ст. 44 ФЗ от 07.07.2003 №126-ФЗ «О связи»
  ...
  template(v-slot:actions)
    .user-list-modal__buttons
      er-button(
        :disabled="!isOk",
        @click="save",
        :loading="isChangingInfo"
        data-ga-category="documents"
        data-ga-label="userslistsave"
      ) Сохранить
      er-button(
        color="gray",
        @click="$emit('close')"
        data-ga-category="documents"
        data-ga-label="userslistcancel"
      ) Отменить
</template>
