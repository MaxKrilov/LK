...
export function removeDuplicates (arr: any) {
  const notDuplicates: any = []
  const duplicates: any = []
  arr.forEach((current: any, index: any) => {
    if (duplicates.includes(index)) return
    notDuplicates['push'](current)
    for (let compInd = index + 1; compInd < arr.length; compInd++) {
      const comparison = arr[compInd]
      ...
      if (valuesEqual) duplicates.push(compInd)
    }
  })
  return notDuplicates
}
// todo РУГАЕТСЯ на дупликат const composedPath
