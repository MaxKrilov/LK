...
export default class DigitalSignature {
     ...
     */
    static async getCertificatesList(cadesplugin) {
        let store;
        try {
     ...
     */
    static getCertificatesListSync(cadesplugin) {
     ...
     */
    static async signCreate(cadesplugin, thumbprint, dataToSign) {
        let store;
        try {
            store = await cadesplugin.CreateObjectAsync('CAPICOM.Store');
            await store.Open(cadesplugin.CAPICOM_CURRENT_USER_STORE, cadesplugin.CAPICOM_MY_STORE, cadesplugin.CAPICOM_STORE_OPEN_MAXIMUM_ALLOWED);
     ...
     */
    static signCreateSync(cadesplugin, thumbprint, dataToSign) {
        const store = cadesplugin.CreateObject('CAPICOM.Store');
        store.Open(cadesplugin.CAPICOM_CURRENT_USER_STORE, cadesplugin.CAPICOM_MY_STORE, cadesplugin.CAPICOM_STORE_OPEN_MAXIMUM_ALLOWED);
        const certificates = store.Certificates;
     ...
     */
    static async signVerify(cadesplugin, signature, origData) {
     ...
     * @param {string} origData
     */
    static signVerifySync(cadesplugin, signature, origData) {
     ...
     */
    static async requestToPresign(document, rawCertificate, visibleSignature) {
        const requestData = {
            Document: document,
     ...
     */
    static async requestToPostsign(signedHashValue, signatureValue, cacheObjectId, visibleSignature) {
        const requestData = {
     ...
     * @return {string}
     */
    static createVisibleSignature(name, certificate) {
        ...
        const issuerName = certificate?.issuerName?.split(',')?.find((item) => ~item.indexOf('CN'))?.slice(3);
     ...
     * @param {string} visibleSignature
     */
    static async signDocument(cadesplugin, document, certificate, visibleSignature) {
        ...
        try {
            const resultSignCreate = await DigitalSignature
                .signCreate(cadesplugin, certificate.thumbprint, document);
            ...
            const resultSignCreateSecond = await DigitalSignature
                .signCreate(cadesplugin, certificate.thumbprint, resultPresign.HashValue);
...
//# sourceMappingURL=digital_signature.js.map